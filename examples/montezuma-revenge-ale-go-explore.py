...
class MyGRUFeaturesExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim: int = 256):
        super().__init__(observation_space, features_dim)
        self.cnn = CNNLayers(in_channels=4)
        self.gru = nn.GRU(input_size=self.cnn.num_elements, hidden_size=features_dim, batch_first=True)

    def forward(self, obs):
        x = self.cnn(obs)
        x = x.unsqueeze(1)          # add sequence dimension
        x, _ = self.gru(x)
        return x[:, -1]             # last GRU output
...
def train(env, exploration_steps: int):
    ...
    def robustify_RNN(state, action, reward, terminated, obs):

        model = PPO(
            policy=ActorCriticPolicy,
            env=env,
            policy_kwargs={
                "features_extractor_class": MyGRUFeaturesExtractor,
    ...
    if True:
        ...
        agent = robustify_RNN(state, action, reward, terminated, obs)
