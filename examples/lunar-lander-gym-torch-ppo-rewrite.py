...
class StateToActionLogits(nn.Module):
    def __init__(self, in_features, out_features, *args, **kwargs):
        ...
        self.net = nn.Sequential(
            tp_torch_utils.layer_init(nn.Linear(in_features, hidden_features)),
            nn.Tanh(),
...
def train(optuna_trial, env, agent, critic, total_timesteps):
    ...
    writer = SummaryWriter(
        f"runs/{gym_environment}_ppo_{optuna_trial.datetime_start.strftime('%Y_%B_%d__%H_%M%p')}_study_{optuna_trial.study.study_name}_trial_no_{optuna_trial.number}"
    ) # TensorBoard
...
def optuna_objective(optuna_trial):
    ...
    train(optuna_trial, env, agent, critic, total_timesteps=4_000_000)
