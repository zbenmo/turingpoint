...
def train(optuna_trial, env, actor: StateToActionDistributionParams, critics: List[StateActionToQValueQuantiles], total_timesteps):
    ...
    def record_video(parcel: dict, videos_path: Path):
        ...
        final_clip.write_videofile(
            videos_path / f"HumanoidStandup-v5-TQC-end-of-step-{parcel['step']}.mp4",
            codec="libx264",
            logger=None
        )
    ...
    def get_train_participants():
        with (tp_tb_utils.Logging(
            path=f"runs/humanoidstandup_tqc_{optuna_trial.datetime_start.strftime('%Y_%B_%d__%H_%M%p')}_study_{optuna_trial.study.study_name}_trial_no_{optuna_trial.number}",
            track=[
                'Mean Rewards/train',
                'Loss(agent)/train',
                'Loss(critic)/train',
