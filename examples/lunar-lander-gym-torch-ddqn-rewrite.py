...
def train(env, agent, target_critic, total_timesteps):
    ...
    with tqdm(total=total_timesteps, desc="training steps") as pbar:
        ...
        def reset_if_needed(parcel: dict):
            if parcel.pop('terminated', False) or parcel.pop('truncated', False):
        ...
        def learn(parcel: dict):
            ...
            parcel['Loss/train'] = loss.item()

                # scheduler.step()

        def init(parcel: dict):
            parcel['step'] = 0

        def check_done(parcel: dict):
            nonlocal pbar
            if parcel['step'] == total_timesteps:
                raise tp.Done
            pbar.update(1)

        def get_epsilon_greedy_action(parcel: dict):
            epsilon = (
                min_epsion
                + (max_epsilon - min_epsion) * ((total_timesteps - parcel['step']) / total_timesteps)
            )
            parcel['epsilon'] = epsilon
            return get_action(parcel, agent=agent, epsilon=parcel['epsilon'])
